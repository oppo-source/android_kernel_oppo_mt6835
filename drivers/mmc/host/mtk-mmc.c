...
static struct msdc_delay_phase get_best_delay_hs400_cmd(struct msdc_host *host, u64 delay,
		u32 max_gears, int full_scan)
{
	int start = 0, len = 0;
	int start_final = 0, len_final = 0;
	...
	pr_err("this emmc manfid is HG, need use 1/2\n");
	if (start_final == 0 && !full_scan)
		final_phase = (start_final + len_final / 3) % max_gears;
 	else
		final_phase = (start_final + len_final / 2) % max_gears;
...
#define HG_VENDOR_MANFID		0xD6
#define HG_C09G00_PROD_NAME "C09G00"
...
static int msdc_execute_hs400_tuning_cmd(struct mmc_host *mmc, struct mmc_card *card)
{
	...
	if ((card != NULL) && (card->cid.manfid == HG_VENDOR_MANFID)
			&& !strncmp((const char*)card->cid.prod_name, HG_C09G00_PROD_NAME, strlen(HG_C09G00_PROD_NAME))) {
		dev_info(host->dev, "card->cid.manfid = %x, cid.prod_name=%s\n", card->cid.manfid, card->cid.prod_name);  //FORESEE and HG manfid is 0xD6
		dev_info(host->dev, "[%s] this is HG emmc, need use 1/2\n", __func__);
		final_rise_delay = get_best_delay_hs400_cmd(host, rise_delay, PAD_DELAY_2CELL_MAX, 0);
	} else {
		dev_info(host->dev, "[%s] this is not HG emmc\n", __func__);
		final_rise_delay = get_best_delay(host, rise_delay, PAD_DELAY_2CELL_MAX, 0);
	}
	...
	if ((card != NULL) && (card->cid.manfid == HG_VENDOR_MANFID)
		...
		dev_info(host->dev, "card->cid.manfid = %x, cid.prod_name=%s\n", card->cid.manfid, card->cid.prod_name);  //FORESEE and HG manfid is 0xD6
		dev_info(host->dev, "[%s] this is HG emmc, need use 1/2\n", __func__);
		final_fall_delay = get_best_delay_hs400_cmd(host, fall_delay, PAD_DELAY_2CELL_MAX, 0);
	} else {
		dev_info(host->dev, "[%s] this is not HG emmc\n", __func__);
		final_fall_delay = get_best_delay(host, fall_delay, PAD_DELAY_2CELL_MAX, 0);
	}
...
/*add for emmc devinfo*/
	...
	dly1_delay = get_best_delay(host, result_dly1, PAD_DELAY_2CELL_MAX, 0);
