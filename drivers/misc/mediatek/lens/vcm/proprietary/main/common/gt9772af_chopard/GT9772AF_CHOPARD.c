...
/*
 * GT9772AF_CHOPARD voice coil motor driver
 *
 *
 */
...
#define AF_DRVNAME "GT9772AF_CHOPARD_DRV"
...
static inline int moveAF(unsigned long a_u4Position)
{
    ...
    int ret = 0;
    LOG_INF("phl move %d", a_u4Position);
...
/* ////////////////////////////////////////////////////////////// */
long GT9772AF_CHOPARD_Ioctl(struct file *a_pstFile, unsigned int a_u4Command,
...
int GT9772AF_CHOPARD_Release(struct inode *a_pstInode, struct file *a_pstFile)
{
    int Ret = 0;
    int pos = g_u4CurrPosition;
    LOG_INF("GT9772AF_CHOPARD_Release phl %d", pos);
    if (*g_pAF_Opened == 2) {
        int pos = g_u4CurrPosition;
        if (pos > 0 && pos <= 1023) {
            while (pos >= 138) {
...
int GT9772AF_CHOPARD_PowerDown(struct i2c_client *pstAF_I2Cclient,
        int *pAF_Opened)
{
...
int GT9772AF_CHOPARD_SetI2Cclient(struct i2c_client *pstAF_I2Cclient,
...
int GT9772AF_CHOPARD_GetFileName(unsigned char *pFileName)
{
