...
static struct wl2866d_i2c_device which_ldo_chip[] = {
	...
	{WL28661D_LDO_I2C_ADDR, WL28661D_CHIP_REV_ADDR, CAMERA_LDO_WL28661D, WL28661D_LDO_EN_ADDR},
...
static void wl2866d_set_en_ldo(enum WL2866D_SELECT ldonum, unsigned int en)
{
    ...
    {
        if((which_ldo_chip[ldo_id].ldoId == CAMERA_LDO_WL2866D) || (which_ldo_chip[ldo_id].ldoId == CAMERA_LDO_WL28661D)) {
            value = (ret|(0x01<<ldonum));
        }
    }
...
static void wl2866d_set_ldo_value(enum WL2866D_SELECT ldonum,unsigned int value)
{
    ...
    {
        case MAIN_DVDD1:
            if (which_ldo_chip[ldo_id].ldoId == CAMERA_LDO_WL2866D || which_ldo_chip[ldo_id].ldoId == CAMERA_LDO_ET5904 || which_ldo_chip[ldo_id].ldoId == CAMERA_LDO_WL28661D) {
                if (value < 600) {
 ...
 * Extern Area
 *****************************************************************************/
int __nocfi wl2866d_set_ldo_enable(enum WL2866D_SELECT ldonum, uint32_t voltage)
{
...
EXPORT_SYMBOL_GPL(wl2866d_set_ldo_enable);

int __nocfi wl2866d_set_ldo_disable(enum WL2866D_SELECT ldonum)
{
    if(ldonum < MAIN_DVDD1 || ldonum > WL2866D_LDO_MAX) {
...
EXPORT_SYMBOL_GPL(wl2866d_set_ldo_disable);
