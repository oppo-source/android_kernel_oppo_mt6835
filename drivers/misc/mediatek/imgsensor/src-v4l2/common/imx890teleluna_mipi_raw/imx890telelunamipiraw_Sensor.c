...
static void imx890teleluna_esd_reset_by_user(struct subdrv_ctx *ctx, u8 *para, u32 *len);
...
static struct subdrv_feature_control feature_control_list[] = {
	...
	{SENSOR_FEATURE_ESD_RESET_BY_USER, imx890teleluna_esd_reset_by_user},
};
...
static int open(struct subdrv_ctx *ctx)
{
	...
	if (g_need_addi_setting == 1) {
		i2c_table_rewrite(ctx, imx890teleluna_addition_setting, ARRAY_SIZE(imx890teleluna_addition_setting));
	}
...
static void imx890teleluna_seamless_switch(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	ae_ctrl->exposure.le_exposure = FINE_INTEG_CONVERT(ae_ctrl->exposure.le_exposure, ctx->s_ctx.mode[ctx->current_scenario_id].fine_integ_line);
	ae_ctrl->exposure.le_exposure = max(ae_ctrl->exposure.le_exposure, (u64)ctx->s_ctx.exposure_min);
	ae_ctrl->exposure.le_exposure = min(ae_ctrl->exposure.le_exposure, (u64)ctx->s_ctx.exposure_max);
	if (cit_step)
		ae_ctrl->exposure.le_exposure = round_up(ae_ctrl->exposure.le_exposure, cit_step);
	...
	DRV_LOG(ctx, "ae_ctrl->exposure.le_exposure + 32 = %u\n", ae_ctrl->exposure.le_exposure+32);
	if (ctx->s_ctx.mode[pre_seamless_scenario_id].hdr_mode == HDR_NONE && prsh_length_lines > (ae_ctrl->exposure.le_exposure + 32)) {
