...
static void omegac1wide_set_max_framerate_by_scenario(struct subdrv_ctx *ctx, u8 *para, u32 *len);
...
static struct subdrv_feature_control feature_control_list[] = {
	...
	{SENSOR_FEATURE_SET_MAX_FRAME_RATE_BY_SCENARIO, omegac1wide_set_max_framerate_by_scenario},
};
...
bool omegac1wide_set_long_exposure(struct subdrv_ctx *ctx, u32 shutter)
{
	...
	if (shutter > (ctx->s_ctx.frame_length_max - ctx->s_ctx.exposure_margin)) {
		...
		set_i2c_buffer(ctx, 0x3060, (subdrv_i2c_rd_u8(ctx, 0x3060) & 0xF8) | (l_shift & 0x7));
...
static void omegac1wide_set_shutter_frame_length_convert(struct subdrv_ctx *ctx, u32 shutter, u32 frame_length)
{
	...
	omegac1wide_set_long_exposure(ctx, shutter);
...
void omegac1wide_set_dummy(struct subdrv_ctx *ctx)
{
}
