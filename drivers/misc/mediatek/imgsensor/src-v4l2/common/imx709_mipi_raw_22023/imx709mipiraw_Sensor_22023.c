 ...
 *============================================================================
 ****************************************************************************/
#include "imx709mipiraw_Sensor_22023.h"

#define IMX709_EEPROM_READ_ID_22023	0xA8
#define IMX709_MAX_OFFSET_22023		0x8000
...
#define IMX709_UNIQUE_SENSOR_ID_22023 0x3CF0
#define IMX709_UNIQUE_SENSOR_ID_LENGHT_22023 11
...
static void imx709_seamless_switch(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void imx709_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void imx709_check_sensor_id(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void imx709_get_sensor_sn(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void imx709_get_otp_checksum_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void imx709_get_unique_sensorid(struct subdrv_ctx *ctx, u8 *para, u32 *len);
...
static void imx709_esd_reset_by_user(struct subdrv_ctx *ctx, u8 *para, u32 *len);
...
static BYTE imx709_unique_id[IMX709_UNIQUE_SENSOR_ID_LENGHT_22023] = { 0 };

static struct eeprom_map_info imx709_eeprom_info[] = {
...
static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_TEST_PATTERN, imx709_set_test_pattern},
	{SENSOR_FEATURE_SEAMLESS_SWITCH, imx709_seamless_switch},
	{SENSOR_FEATURE_CHECK_SENSOR_ID, imx709_check_sensor_id},
	{SENSOR_FEATURE_GET_EEPROM_COMDATA, imx709_get_sensor_sn},
	{SENSOR_FEATURE_GET_SENSOR_OTP_ALL, imx709_get_otp_checksum_data},
	{SENSOR_FEATURE_GET_UNIQUE_SENSORID, imx709_get_unique_sensorid},
	{SENSOR_FEATURE_SET_AWB_GAIN, feedback_awbgain},
	{SENSOR_FEATURE_ESD_RESET_BY_USER, imx709_esd_reset_by_user},
};
...
static struct subdrv_mode_struct mode_struct[] = {
	...
	},
	{
		...
		.seamless_switch_mode_setting_table = imx709_seamless_normal_video,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(imx709_seamless_normal_video),
		.hdr_group = 1,
		.hdr_mode = HDR_NONE,
		.pclk = 864000000,
		.linelength = 11760,
	...
	},
	{
		...
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = imx709_seamless_custom4,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(imx709_seamless_custom4),
		.hdr_group = 1,
...
static struct subdrv_static_ctx static_ctx = {
	.sensor_id = IMX709_SENSOR_ID_22023,
...
const struct subdrv_entry imx709_mipi_raw_22023_entry = {
	.name = "imx709_mipi_raw_22023",
	.id = IMX709_SENSOR_ID_22023,
	.pw_seq = pw_seq,
...
static unsigned int read_imx709_eeprom_info(struct subdrv_ctx *ctx, kal_uint16 meta_id,
	BYTE *data, int size)
{
	kal_uint16 addr;
	int readsize;

	if (meta_id != imx709_eeprom_info[meta_id].meta)
		return -1;

	if (size != imx709_eeprom_info[meta_id].size)
		return -1;

	addr = imx709_eeprom_info[meta_id].start;
	readsize = imx709_eeprom_info[meta_id].size;
...
static void read_module_data(struct subdrv_ctx *ctx)
{
	...
	// ctx->s_ctx.module_id = (kal_uint16)(imx709_common_data[1] << 8) |
	...
	read_imx709_eeprom_info(ctx, EEPROM_META_AF_CODE,
		&(imx709_common_data[25]), 6);
	read_imx709_eeprom_info(ctx, EEPROM_META_AF_FLAG,
		&(imx709_common_data[33]), 1);

	for (idx = 0; idx < 36; idx = idx + 4)
		DRV_LOG(ctx, "otp data: %02x %02x %02x %02x\n", imx709_common_data[idx],
			imx709_common_data[idx + 1], imx709_common_data[idx + 2],
...
{
	u8 i = 0;
	DRV_LOGE(ctx, "read sensor unique sensorid");
	while (ctx->s_ctx.i2c_addr_table[i] != 0xFF) {
		ctx->i2c_write_id = ctx->s_ctx.i2c_addr_table[i];
		adaptor_i2c_rd_p8(ctx->i2c_client, ctx->i2c_write_id >> 1,
		IMX709_UNIQUE_SENSOR_ID_22023, &(imx709_unique_id[0]), IMX709_UNIQUE_SENSOR_ID_LENGHT_22023);
		i++;
	}
}
...
{
	u32 *feature_return_para_32 = (u32 *)para;
	DRV_LOGE(ctx, "get unique sensorid");
	memcpy(feature_return_para_32, imx709_unique_id,
		IMX709_UNIQUE_SENSOR_ID_LENGHT_22023);
	DRV_LOGE(ctx, "para :%x, get unique sensorid", *para);
}

static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
                    BYTE *data, int size)
{
	if (adaptor_i2c_rd_p8(ctx->i2c_client, IMX709_EEPROM_READ_ID_22023 >> 1,
		addr, data, size) < 0) {
		return false;
	}
	return true;
...
static void imx709_check_sensor_id(struct subdrv_ctx *ctx, u8 *para, u32 *len) {
	imx709_get_imgsensor_id(ctx, (u32*)para);
}
...
static int imx709_open(struct subdrv_ctx *ctx)
{
	u32 sensor_id = 0;
	u32 scenario_id = 0;
	u32 rmsc_ip_setting_len = ARRAY_SIZE(imx709_rmsc_ip_setting);
	...
	i2c_table_write(ctx, imx709_rmsc_ip_setting, rmsc_ip_setting_len);
