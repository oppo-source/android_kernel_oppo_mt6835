 ...
 * Filename:
 * ---------
 *     gc02m1bmipimono_alphab5_Sensor.c
 *
 * Project:
 * --------
 *     ALPS
...
{
	return (((read_cmos_sensor(0xf0) << 8) | read_cmos_sensor(0xf1)) + SENSOR_ID_OFFSET_ALPHAB5);
}
...
static kal_uint16 addr_data_pair_custom1_gc02m1b[] = {
    0x41, 0x06,
    0x42, 0x3c,
};
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				printk("debug GC02M1B(Y1&Y6) i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
				imgsensor_info.module_id = 0x00;
				if (deviceInfo_register_value == 0x00) {
					register_imgsensor_deviceinfo("Cam_r1", DEVICE_VERSION_ALPHAB5_GC02M1B, imgsensor_info.module_id);
					deviceInfo_register_value=0x01;
				}
				return ERROR_NONE;
			}
			printk("debug GC02M1B Read sensor id fail, write id: 0x%x, id: 0x%x, offset val is 20 \n", imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
		retry = 2;
