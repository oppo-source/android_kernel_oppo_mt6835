...
#include "sc202csmipiraw_bl5s_Sensor.h"
#include "imgsensor_common.h"


/****************************Modify Following Strings for Debug****************************/
#define PFX "sc202cs_mipi_bl5s"
...
static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = SC202CS_SENSOR_ID_BL5S,
...
{
	return (((read_cmos_sensor(0x3107) << 8) | read_cmos_sensor(0x3108)) + SENSOR_ID_OFFSET_BL5S);
}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        LOG_INF("SC202CS write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
        retry--;
        } while(retry > 0);
        i++;
        retry = 2;
...
UINT32 SC202CS_MIPI_RAW_BL5S_SensorInit(struct SENSOR_FUNCTION_STRUCT  **pfFunc)
{
