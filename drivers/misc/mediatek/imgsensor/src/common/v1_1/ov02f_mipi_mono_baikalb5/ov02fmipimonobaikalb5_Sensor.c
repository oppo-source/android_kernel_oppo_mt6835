...
#include "ov02fmipimonobaikalb5_Sensor.h"
...
#define DEVICE_VERSION_22633_OV02F    "ov02f"
...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = OV02F_SENSOR_ID_BAIKALB5,
...
static kal_uint32 return_sensor_id(void)
{
    ...
    return (read_cmos_sensor(0x0200) << 8) | read_cmos_sensor(0x0300) + SENSOR_ID_OFFSET_BAIKALB5;
}
        ...
        do {
            *sensor_id = return_sensor_id();
            if (*sensor_id == imgsensor_info.sensor_id) {
                pr_info("chaorong_kernel_if");
                pr_info("[get_imgsensor_id] ov02f Read sensor id Success, write id:0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
                imgsensor_info.module_id = 0x00;
                if (deviceInfo_register_value == 0x00) {
                    register_imgsensor_deviceinfo("Cam_r1", DEVICE_VERSION_22633_OV02F, imgsensor_info.module_id);
                    deviceInfo_register_value=0x54;
                }
                return ERROR_NONE;
            }
            pr_info("[get_imgsensor_id] ov02f Read sensor id fail, write id:0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
            retry--;
        } while (retry > 0);
        i++;
        retry = 2;
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            ...
            if (sensor_id == 0x5602) {
                ...
                pr_info("[Open] ov02f Read sensor id Success, write id:0x%x, id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
                break;
            }
            pr_info("[Open] ov02f Read sensor id fail, write id:0x%x, id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
            retry--;
        } while (retry > 0);
        i++;
        if (sensor_id == imgsensor_info.sensor_id)
