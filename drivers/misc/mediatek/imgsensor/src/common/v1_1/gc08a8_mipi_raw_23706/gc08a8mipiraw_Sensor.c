...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = GC08A8_SENSOR_ID_23706,
	.module_id = 0x07,
...
// sensorid : [30 ~ 33]
	...
	kal_uint32 sn_starAddr = 0x17B8;
	kal_uint32 vcmAddr = 0x15F8;
	kal_uint32 lensAddr = 0x15E8;

	memset(gc08a8_common_data, 0, sizeof(gc08a8_common_data));
	// QR
	for(idx = 0; idx < sn_length; idx++)
	{
		gc08a8_common_data[8 + idx] = read_gc08a8_avatarl5_eeprom_module(sn_starAddr + idx);
	}
	//vcm
	gc08a8_common_data[40] = read_gc08a8_avatarl5_eeprom_module(vcmAddr);
	gc08a8_common_data[41] = read_gc08a8_avatarl5_eeprom_module(vcmAddr + 1);
	//lensid
	gc08a8_common_data[44] = read_gc08a8_avatarl5_eeprom_module(lensAddr);
	gc08a8_common_data[45] = read_gc08a8_avatarl5_eeprom_module(lensAddr + 1);

	for (idx = 0; idx < CAMERA_EEPPROM_COMDATA_LENGTH; idx = idx + 4)
		printk("cam data, idx: %02x %02x %02x %02x %d\n",
		       gc08a8_common_data[idx],
		       gc08a8_common_data[idx + 1],
		       gc08a8_common_data[idx + 2],
		       gc08a8_common_data[idx + 3],
			   idx);
}*/
/*
...
#define PAGE_NUM 3
//static int page_addr_start[PAGE_NUM] = {0x15A8,0x5338,0x92C8};
//static int page_addr_end[PAGE_NUM] = {0x5330,0x92C0,0xD050};
//static int page_addr[PAGE_NUM] = {0x01,0x13,0x37};
//static int sn_flag_addr_tbl[MAX_GROUP_NUM] = {0x1870, 0x5800, 0x9590};
//static int sn_data_addr_tbl[MAX_GROUP_NUM] = {0x17B8, 0x5748, 0x94D8};

//unsigned char gc08a8_data_sn[SN_DATA_SIZE+2] = {0};/*Add flag and check sum*/
//unsigned char gc08a8_sn_valid = 0;
#if 0
static bool gc08a8_sensor_otp_init(int group_idx)
{
...
static int read_gc08a8_sn_info(void)
{
	...
	for(i = 0; i < MAX_GROUP_NUM; i++){
		write_cmos_sensor_8bit(0x15A0, (sn_flag_addr_tbl[i])&0xff);
		write_cmos_sensor_8bit(OTP_RW_FLAG, 0x01);
	...
	if(sn_start_addr != 0){
		...
		{
			gc08a8_data_sn[i] = read_cmos_sensor(sn_start_addr+i);
		}

		for(i = 0; i < SN_DATA_SIZE; i++){
			check_sum_sn_cal += gc08a8_data_sn[i];
		}
		pr_info("check_sum_sn_cal =0x%x \n",check_sum_sn_cal);
		check_sum_sn = gc08a8_data_sn[SN_DATA_SIZE+1];
		check_sum_sn_cal = check_sum_sn_cal % 255;

		pr_info("=== gc08a8 SN check_sum_sn=0x%x, check_sum_sn_cal=0x%x ===\n", check_sum_sn, check_sum_sn_cal);
		if(check_sum_sn == check_sum_sn_cal){
			pr_info("check_sum_sn success!\n");
			for (i = 0; i < SN_DATA_SIZE; i++) {
				gc08a8_common_data[8+i] = (kal_uint8)gc08a8_data_sn[i];
			printk("sn:%x",gc08a8_data_sn[i]);
			}
		}else{
			return 0;
...
#endif
#if 0
static bool gc08a8_sn_get(){
	int ret = 0,i = 0;
	for(i = 0 ; i < PAGE_NUM;i++){
		ret = gc08a8_sensor_otp_init(i);
		if (!ret){
			pr_err("gc08a8_sensor_otp_init false");
			return FALSE;
		}
		ret = read_gc08a8_sn_info();
		if (!ret){
			pr_err("read_gc08a8_sn_info false");
			return FALSE;
		}
		return TRUE;
	}
...
kal_uint16 gc08a8_23706_init_addr_data[] = {
	0x031c, 0x60,
	0x0337, 0x04,
	0x0335, 0x51,
	0x0336, 0x70,
...
static void sn_test(void)
{
	...
	{
		printk("-----WHHH gi2c.inst[1].pi2c_client is NULL!!");
		return;
	}
	else
	{
		printk("-----WHHH gi2c.inst[1].pi2c_client is not NULL addr: 0x%x!!",gi2c.inst[1].pi2c_client->addr);
	}
	...
	for(j = 0;j < 31;j++)
		printk("gc08a8_common_data[%d]:0x%x",j,gc08a8_common_data[j]);

	for(j = 0;j < 31;j++)
		printk("otp_data[%d]:0x%x",j,otp_data[j]);
}

static void sensor_init(void)
{
	pr_debug("[%s] init_start\n", __func__);
	table_write_cmos_sensor(gc08a8_23706_init_addr_data,
		sizeof(gc08a8_23706_init_addr_data)/sizeof(kal_uint16));
	pr_debug("[%s] init_End\n", __func__);
}	/*	  sensor_init  */
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id();
			printk("23706_front_gc08a8 get_imgsensor_id : 0x%x,imgsensor_info.sensor_id : 0x%x\n", *sensor_id,imgsensor_info.sensor_id);
			if (*sensor_id == (imgsensor_info.sensor_id - 1)) {
				if ((read_gc08a8_version()) < GC08A8_ECO_VALUE) {
					*sensor_id = 0xFFFFFFFF;
					printk("23706_front_gc08a8 get_imgsensor_id failed: 0x%x\n", *sensor_id);
					...
					printk("23706_front_gc08a8 get_imgsensor_id success: 0x%x\n", *sensor_id);
					return ERROR_NONE;
				}
			}
			retry--;
...
static kal_uint32 open(void)
{
	...
	//printk("---WHH before open sn test!");
	//sn_test();
	/* initail sequence write in  */
	sensor_init();
	//printk("---WHH open sn test!");
	//sn_test();
	spin_lock(&imgsensor_drv_lock);
				 ...
				 UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_EEPROM_COMDATA:
		memcpy(feature_return_para_32, gc08a8_common_data, CAMERA_EEPPROM_COMDATA_LENGTH);
			for(j = 0;j < 25;j++)
		printk("-----WHH SENSOR_FEATURE_GET_EEPROM_COMDATA gc08a8_common_data[%d]:0x%x",j,gc08a8_common_data[j]);
