...
#include "ov08d10mipiraw_barley_Sensor.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV08D10_SENSOR_ID_BARLEY,
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[7] = {
	...
	{  3264, 2448,  0, 0, 3264, 2448, 3264, 2448, 0000, 0000, 3264, 2448,  0, 0, 3264, 2448},/*custom2:binning size*/
};
...
static kal_uint16 barley_ov08d10_custom2_setting[] =  {
    #if 0
    // 1632x1224 settings
	0xfd, 0x00,
	0x20, 0x0e,
...
static void custom2_setting(void)
{
	...
	write_cmos_sensor(0x20, 0x0e);
	mdelay(3);
	table_write_cmos_sensor(barley_ov08d10_custom2_setting,
		sizeof(barley_ov08d10_custom2_setting) / sizeof(kal_uint16));
}
//#ifdef OPLUS_FEATURE_CAMERA_COMMON
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		*sensor_id = return_sensor_id();
		printk("barley_FRONT_OV08D10 get_imgsensor_id : 0x%x\n", *sensor_id);
		if (*sensor_id == imgsensor_info.sensor_id) {
			...
			//	printk("barley_FRONT_OV08D10 get_imgsensor_id failed: 0x%x\n", *sensor_id);
				...
				LOG_INF("barley_FRONT_OV08D10 get_imgsensor_id success: 0x%x\n", *sensor_id);
				return ERROR_NONE;
			}
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id();
			if (sensor_id == imgsensor_info.sensor_id) {
				LOG_INF("barley_OV08D10 open success: 0x%x\n", sensor_id);
				break;
			}
			retry--;
		} while(retry > 0);
...
static kal_uint32 custom2(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
		MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	pr_info("[barley_ov08d10] custom2 mode start\n");
