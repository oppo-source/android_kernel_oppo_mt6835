...
#include "ov02b1bmipimono23281_Sensor.h"
...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = OV02B1B_SENSOR_ID_23281,
            ...
            *sensor_id = return_sensor_id();
            if (*sensor_id == OV02B10_SENSOR_ID) {
                *sensor_id = imgsensor_info.sensor_id;
                pr_info("[get_imgsensor_id] Ov02b1b Read sensor id Success, write id:0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
                if (deviceInfo_register_value == 0x00) {
                    deviceInfo_register_value = 0x01;
                }
                return ERROR_NONE;
            }
            pr_info("[get_imgsensor_id] Ov02b1b Read sensor id fail, write id:0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
            retry--;
        } while (retry > 0);
        i++;
        retry = 2;
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            ...
            if (sensor_id == OV02B10_SENSOR_ID) {
                sensor_id = imgsensor_info.sensor_id;
                pr_info("[Open] Ov02b1b Read sensor id Success, write id:0x%x, id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
                break;
            }
            pr_info("[Open] Ov02b1b Read sensor id fail, write id:0x%x, id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
            retry--;
        } while (retry > 0);
        i++;
        if (sensor_id == imgsensor_info.sensor_id)
