...
#include "sc520csmipiraw_alphab5_Sensor.h"

#define LEFT_SIZE (0x4000)
#define SC520CS_EEPROM_I2C_ADDR 0xA0
#define DEVICE_VERSION_SC520CS_ALPHAB5 "sc520cs_alphab5"
...
char g_alphab5_otp_buf[3][MAX_EEPROM_BYTE] = {{0}, {0}, {0}};
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = SC520CS_SENSOR_ID_ALPHAB5,
...
{
	kal_uint32 sensor_id = ((read_cmos_sensor(0x3107) << 8) | read_cmos_sensor(0x3108)) + SENSOR_ID_OFFSET_ALPHAB5;
...
int sc520cs_alphab5_read_data(u16 addr, u8 *data);

OTP_MAP sc520cs_otp_map_alphab5 = {
		.module_name = "SC520CS_ALPHAB5",
		.group_num = 3,
		.group_addr_info_num = 2,
		.readFunc = sc520cs_alphab5_read_data,
		.group_info = {
						{"awb",
							{
...
static kal_uint16 read_sc520cs_alphab5_eeprom_module(kal_uint32 addr)
{
	kal_uint16 get_byte = 0;
	char pusendcmd[2] = { (char)(addr >> 8), (char)(addr & 0xFF) };
...
static void read_sc520cs_module_data()
{
	...
	{
		sc520cs_common_data[8 + idx] = read_sc520cs_alphab5_eeprom_module(sn_starAddr + idx);
	}
	//vcm
	sc520cs_common_data[40] = read_sc520cs_alphab5_eeprom_module(vcmAddr);
	sc520cs_common_data[41] = read_sc520cs_alphab5_eeprom_module(vcmAddr + 1);
	//lensid
	sc520cs_common_data[44] = read_sc520cs_alphab5_eeprom_module(lensAddr);
	sc520cs_common_data[45] = read_sc520cs_alphab5_eeprom_module(lensAddr + 1);
...
char sub_alphab5_camera_info[20] = {0};
EXPORT_SYMBOL(sub_alphab5_camera_info);
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	kal_uint8 i = 0;
	kal_uint8 retry = 2;
	LOG_INF("[sc520cs alphab5 get_imgsensor_id] ");
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
			...
			do {
				*sensor_id =return_sensor_id();
				sprintf(sub_alphab5_camera_info, "sc520cs");

				if (*sensor_id == imgsensor_info.sensor_id) {
					LOG_INF("i2c write id  : 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);

					if(deviceInfo_register_value == 0x00) {
						register_imgsensor_deviceinfo("Cam_f", DEVICE_VERSION_SC520CS_ALPHAB5, imgsensor_info.module_id);
