...
#include "imx355mipiraw23281_Sensor.h"
...
static BYTE imx355_common_data[CAMERA_EEPPROM_COMDATA_LENGTH] = { 0 };
...
static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = IMX355_SENSOR_ID_23281,
...
static kal_uint16 read_imx355_eeprom_module(kal_uint32 addr)
{
    kal_uint16 get_byte = 0;
    char pusendcmd[2] = { (char)(addr >> 8), (char)(addr & 0xFF) };
...
static void read_imx355_module_data()
{
    // length   : 64
    // QR       : [8 ~ 26]
    // vcm      : [40 ~ 41]
    ...
    memset(imx355_common_data, 0, sizeof(imx355_common_data));
    // QR
    for(idx = 0; idx < sn_length; idx++)
    {
       imx355_common_data[8 + idx] = read_imx355_eeprom_module(sn_starAddr + idx);
    }
    // Lens ID
    imx355_common_data[44] = read_imx355_eeprom_module(lensAddr);
    imx355_common_data[45] = read_imx355_eeprom_module(lensAddr + 1);
    // Vcm ID
    imx355_common_data[40] = read_imx355_eeprom_module(vcmAddr);
    imx355_common_data[41] = read_imx355_eeprom_module(vcmAddr + 1);

    for (idx = 0; idx < CAMERA_EEPPROM_COMDATA_LENGTH; idx = idx + 4)
    {
        pr_info("imx355 common cam data : %02x %02x %02x %02x\n",
           imx355_common_data[idx],
           imx355_common_data[idx + 1],
            ...
            *sensor_id = return_sensor_id();
            if (*sensor_id == IMX355_SENSOR_ID) {
                *sensor_id = imgsensor_info.sensor_id;
                pr_info("[get_imgsensor_id] Imx355 Read sensor id Success, write id:0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
                if (deviceInfo_register_value == 0x00) {
                    read_imx355_module_data();
            ...
            pr_info("[get_imgsensor_id] Imx355 Read sensor id fail, write id:0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
            retry--;
        } while (retry > 0);
        i++;
        retry = 2;
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            sensor_id = return_sensor_id();
            if (sensor_id == IMX355_SENSOR_ID) {
                pr_info("[Open] Imx355 Read sensor id Success, write id:0x%x, id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
                sensor_id = imgsensor_info.sensor_id;
                break;
            }
            pr_info("[Open] Imx355 Read sensor id fail, write id:0x%x, id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
            retry--;
        } while (retry > 0);
        i++;
        if (sensor_id == imgsensor_info.sensor_id)
                 ...
                 UINT8 *feature_para, UINT32 *feature_para_len)
{
    ...
    case SENSOR_FEATURE_GET_EEPROM_COMDATA:
        memcpy(feature_return_para_32, imx355_common_data, CAMERA_EEPPROM_COMDATA_LENGTH);
