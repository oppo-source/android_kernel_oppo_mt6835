 ...
 * Filename:
 * ---------
 *     gc02m1bmipimono_baikall5_Sensor.c
 *
 * Project:
 * --------
 *     ALPS
...
#include "gc02m1bmipimono_baikall5_Sensor.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = GC02M1B_SENSOR_ID_BAIKALL5,
...
{
	return (((read_cmos_sensor(0xf0) << 8) | read_cmos_sensor(0xf1)) + SENSOR_ID_OFFSET_BAIKALL5);
}
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				printk("GC02M1B(Y1&Y6) i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
				return ERROR_NONE;
			}
