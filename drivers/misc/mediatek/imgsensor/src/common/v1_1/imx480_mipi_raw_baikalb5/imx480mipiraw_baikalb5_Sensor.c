...
#include "imx480mipiraw_baikalb5_Sensor.h"
...
#define IMX480_BAIKALB5_EEPROM_I2C_ADDR 0xA8
...
#define DEVICE_VERSION  "imx480"
static BYTE imx480_baikalb5_common_data[CAMERA_EEPPROM_COMDATA_LENGTH] = { 0 };
...
static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = IMX480_SENSOR_ID_BAIKALB5,
...
{
    char pusendcmd[2] = {(char)(addr >> 8), (char)(addr & 0xFF) };
    iReadRegI2C(pusendcmd, 2, (u8 *)table, size,IMX480_BAIKALB5_EEPROM_I2C_ADDR);
}
...
static kal_uint16 read_imx480_baikalb5_eeprom_module(kal_uint32 addr)
{
	kal_uint16 get_byte = 0;
	char pusendcmd[2] = { (char)(addr >> 8), (char)(addr & 0xFF) };

	iReadRegI2C(pusendcmd, 2, (u8 *) &get_byte, 1, IMX480_BAIKALB5_EEPROM_I2C_ADDR);

	return get_byte;
}

static void read_imx480_baikalb5_module_data()
{
// imx480_baikalb5_common_data
// length   : 64
// QR       : [8 ~ 26]
// vcm      : [40 ~ 41]
// lensid   : [44 ~ 45]
// sensorid : [30 ~ 33]
	...
	memset(imx480_baikalb5_common_data, 0, sizeof(imx480_baikalb5_common_data));
	// QR
	for(idx = 0; idx < sn_length; idx++)
	{
		imx480_baikalb5_common_data[8 + idx] = read_imx480_baikalb5_eeprom_module(sn_starAddr + idx);
	}
	//vcm
	imx480_baikalb5_common_data[40] = read_imx480_baikalb5_eeprom_module(vcmAddr);
	imx480_baikalb5_common_data[41] = read_imx480_baikalb5_eeprom_module(vcmAddr + 1);
	//lensid
	imx480_baikalb5_common_data[44] = read_imx480_baikalb5_eeprom_module(lensAddr);
	imx480_baikalb5_common_data[45] = read_imx480_baikalb5_eeprom_module(lensAddr + 1);

	for (idx = 0; idx < CAMERA_EEPPROM_COMDATA_LENGTH; idx = idx + 4)
		pr_err("imx480eeprom_moduleinfo cam data, idx: %02x %02x %02x %02x %d\n",
		       imx480_baikalb5_common_data[idx],
		       imx480_baikalb5_common_data[idx + 1],
		       imx480_baikalb5_common_data[idx + 2],
		       imx480_baikalb5_common_data[idx + 3],
			   idx);
}
        ...
        *sensor_id = return_sensor_id();
        if (*sensor_id == 0x0471) {
            *sensor_id = imgsensor_info.sensor_id;
            ...
            read_imx480_baikalb5_module_data();
            return ERROR_NONE;
        }
        pr_err("BB5 IMX480 Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
        retry--;
    } while (retry > 0);
    i++;
    retry = 2;
                             ...
                             UINT8 *feature_para,UINT32 *feature_para_len)
{
    ...
    case SENSOR_FEATURE_GET_EEPROM_COMDATA:
        memcpy(feature_return_para_32, imx480_baikalb5_common_data, CAMERA_EEPPROM_COMDATA_LENGTH);
