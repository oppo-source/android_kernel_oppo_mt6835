...
#define PFX "ov50d40_st_camera_sensor"
...
#include "ov50d40stmipiraw_alphab5_Sensor.h"
#include "ov50d40_st_Sensor_alphab5_setting.h"
...
static kal_uint16 ov50d40_st_table_write_cmos_sensor(
...
static void sensor_init(void)
{
    ...
    ov50d40_st_table_write_cmos_sensor(
        addr_data_pair_init_ov50d40_st,
        sizeof(addr_data_pair_init_ov50d40_st) / sizeof(kal_uint16));
...
static void preview_setting(void)
{
    ...
    _length = sizeof(addr_data_pair_preview_ov50d40_st) / sizeof(kal_uint16);

    ov50d40_st_table_write_cmos_sensor(
        addr_data_pair_preview_ov50d40_st,
        _length);
...
static void capture_setting(kal_uint16 currefps)
{
    ...
    _length = sizeof(addr_data_pair_capture_ov50d40_st) / sizeof(kal_uint16);
    ov50d40_st_table_write_cmos_sensor(
        addr_data_pair_capture_ov50d40_st,
        _length);
}
...
static void normal_video_setting(kal_uint16 currefps)
{
    ...
    _length = sizeof(addr_data_pair_video_ov50d40_st) / sizeof(kal_uint16);
    ov50d40_st_table_write_cmos_sensor(
        addr_data_pair_video_ov50d40_st,
        _length);
...
static void hs_video_setting(void)
{
    ...
    _length = sizeof(addr_data_pair_hs_video_ov50d40_st) / sizeof(kal_uint16);
        ov50d40_st_table_write_cmos_sensor(
        addr_data_pair_hs_video_ov50d40_st,
        _length);
...
static void slim_video_setting(void)
{
    ...
    _length = sizeof(addr_data_pair_slim_video_ov50d40_st) / sizeof(kal_uint16);
        ov50d40_st_table_write_cmos_sensor(
        addr_data_pair_slim_video_ov50d40_st,
        _length);
}
...
{
    char pusendcmd[2] = {(char)(addr >> 8), (char)(addr & 0xFF) };
    iReadRegI2C(pusendcmd, 2, (u8 *)table, size, OV50D40_ST_EEPROM_I2C_ADDR);
}
...
static kal_uint16 read_ov50d40_st_eeprom_module(kal_uint32 addr)
{
    kal_uint16 get_byte = 0;
    char pusendcmd[2] = { (char)(addr >> 8), (char)(addr & 0xFF) };

    iReadRegI2C(pusendcmd, 2, (u8 *) &get_byte, 1, OV50D40_ST_EEPROM_I2C_ADDR);
    return get_byte;
}
...
/* OVPD-1:450 Bytes */
static kal_uint16 ov50d_st_OVPD_setting[450*2];
...
static void read_Eeprom_OVPD(void)
{
    ...
    for (i = 0; i < 450; i ++) {
        ov50d_st_OVPD_setting[2*i] = senaddr_ovpd+i;
        ov50d_st_OVPD_setting[2*i+1] = read_ov50d40_st_eeprom_module((addr_ovpd+i));
    }
}
...
static void write_sensor_OVPD(void)
{
    ...
    ov50d_burst_write_cmos_sensor(ov50d_st_OVPD_setting,
        sizeof(ov50d_st_OVPD_setting) / sizeof(kal_uint16));

    LOG_INF("%s end\n", __func__);

}

static void read_ov50d40_st_module_data()
{
    // length   : 64
    // QR       : [8 ~ 26]
    // vcm      : [40 ~ 41]
    ...
    memset(ov50d40_st_common_data, 0, sizeof(ov50d40_st_common_data));
    // QR
    for(idx = 0; idx < sn_length; idx++)
    {
       ov50d40_st_common_data[8 + idx] = read_ov50d40_st_eeprom_module(sn_starAddr + idx);
    }
    // Vcm ID
    ov50d40_st_common_data[40] = read_ov50d40_st_eeprom_module(vcmAddr);
    ov50d40_st_common_data[41] = read_ov50d40_st_eeprom_module(vcmAddr + 1);
    // Lens ID
    ov50d40_st_common_data[44] = read_ov50d40_st_eeprom_module(lensAddr);
    ov50d40_st_common_data[45] = read_ov50d40_st_eeprom_module(lensAddr + 1);
    // Macro
    ov50d40_st_common_data[48] = read_ov50d40_st_eeprom_module(macAddr);
    ov50d40_st_common_data[49] = read_ov50d40_st_eeprom_module(macAddr + 1);
    // Inf
    ov50d40_st_common_data[52] = read_ov50d40_st_eeprom_module(infAddr);
    ov50d40_st_common_data[53] = read_ov50d40_st_eeprom_module(infAddr + 1);

    for (idx = 0; idx < CAMERA_EEPPROM_COMDATA_LENGTH; idx = idx + 4)
    {
        pr_info("ov50d40 st common cam data : %02x %02x %02x %02x\n",
           ov50d40_st_common_data[idx],
           ov50d40_st_common_data[idx + 1],
           ov50d40_st_common_data[idx + 2],
           ov50d40_st_common_data[idx + 3]);
    }
}
...
static kal_uint32 return_sensor_id(void)
{
    return (((read_cmos_sensor(0x300a) << 16) | (read_cmos_sensor(0x300b) << 8) |
              read_cmos_sensor(0x300c))) + SENSOR_ID_OFFSET_ALPHAB5;
}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        spin_unlock(&imgsensor_drv_lock);
        do {
            imgsensor_info.module_id = read_ov50d40_st_eeprom_module(MODULE_ID_OFFSET);
            if (shineTech_Vendorid == imgsensor_info.module_id){
                *sensor_id = (return_sensor_id()+0x5);
                pr_info("OV50D40 is shine tech sensorid is  0x%x , imgsensor_info.module_id is 0x%x\n",*sensor_id , imgsensor_info.module_id);
            } else {
                pr_info("OV50D40 is not shine tech sensorid is  0x%x , imgsensor_info.module_id is 0x%x\n",*sensor_id , imgsensor_info.module_id);
                *sensor_id = 0xFFFFFFFF;
                return ERROR_SENSOR_CONNECT_FAIL;
            }
            if (*sensor_id == imgsensor_info.sensor_id) {
                pr_info("[get_imgsensor_id] debug OV50D40 shine tech alphab5 Read sensor id Success, write id:0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
                if(deviceInfo_register_value == 0x00) {
                    register_imgsensor_deviceinfo("Cam_r", DEVICE_VERSION_ALPHAB5_OV50D40_ST, imgsensor_info.module_id);
                    read_ov50d40_st_module_data();
                    deviceInfo_register_value = 0x07;
                }
                if (otp_data[0] == 0) {
            ...
            pr_err("[get_imgsensor_id] OV50D40 shine tech Read sensor id fail, write id:0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
            retry--;
        } while (retry > 0);
            i++;
        retry = 2;
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            ...
            sensor_id = (return_sensor_id()+0x5);
            if (sensor_id == imgsensor_info.sensor_id) {
                pr_info("[Open] OV50D40 shine tech Read sensor id Success, write id:0x%x, id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
                break;
            }
            pr_info("[Open] OV50D40 shine tech Read sensor id fail, write id:0x%x, id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
            retry--;
        } while (retry > 0);
            i++;
        if (sensor_id == imgsensor_info.sensor_id)
            ...
            UINT8 *feature_para, UINT32 *feature_para_len)
{
    ...
    *(feature_data + 1) = imgsensor_info.min_shutter;
    ...
    case SENSOR_FEATURE_GET_EEPROM_COMDATA:
        memcpy(feature_return_para_32, ov50d40_st_common_data, CAMERA_EEPPROM_COMDATA_LENGTH);
