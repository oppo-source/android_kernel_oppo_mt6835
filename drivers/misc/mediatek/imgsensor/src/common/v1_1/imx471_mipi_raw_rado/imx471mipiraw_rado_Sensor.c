...
#include "imx471mipiraw_rado_Sensor.h"
...
static BYTE imx471_rado_common_data[CAMERA_EEPPROM_COMDATA_LENGTH] = { 0 };

static DEFINE_SPINLOCK(imgsensor_drv_lock);

static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = IMX471_SENSOR_ID_RADO,
...
static kal_uint16 read_imx471_rado_eeprom_module(kal_uint32 addr)
{
	kal_uint16 get_byte = 0;
	char pusendcmd[2] = { (char)(addr >> 8), (char)(addr & 0xFF) };
...
static void read_imx471_rado_module_data()
{
...
// sensorid : [30 ~ 33]
	...
	memset(imx471_rado_common_data, 0, sizeof(imx471_rado_common_data));
	// QR
	for(idx = 0; idx < sn_length; idx++)
	{
		imx471_rado_common_data[8 + idx] = read_imx471_rado_eeprom_module(sn_starAddr + idx);
	}
	//vcm
	...
	for (idx = 0; idx < CAMERA_EEPPROM_COMDATA_LENGTH; idx = idx + 4)
		printk("cam data, idx: %02x %02x %02x %02x %d\n",
		       imx471_rado_common_data[idx],
		       imx471_rado_common_data[idx + 1],
		       imx471_rado_common_data[idx + 2],
		       imx471_rado_common_data[idx + 3],
			   idx);
}
                             ...
                             UINT8 *feature_para,UINT32 *feature_para_len)
{
    ...
    case SENSOR_FEATURE_GET_EEPROM_COMDATA:
        memcpy(feature_return_para_32, imx471_rado_common_data, CAMERA_EEPPROM_COMDATA_LENGTH);
