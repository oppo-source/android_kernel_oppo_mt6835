...
#include "sc202csmipiraw_alphab5_Sensor.h"
#include "imgsensor_common.h"


/****************************Modify Following Strings for Debug****************************/
#define PFX "sc202cs_mipi_alphab5"
...
#define DEVICE_VERSION_22633_SC202CS    "sc202cs_alphab5"
static kal_uint8 deviceInfo_register_value = 0x00;

static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = SC202CS_SENSOR_ID_ALPHAB5,
...
{
	return (((read_cmos_sensor(0x3107) << 8) | read_cmos_sensor(0x3108)) + SENSOR_ID_OFFSET_ALPHAB5);
}
			...
			*sensor_id = return_sensor_id();
            pr_err("cwd return_sensor_id:0x%x\n",*sensor_id);
             if (*sensor_id == imgsensor_info.sensor_id) {
                LOG_INF("zefdebug address is ok,and id is ok, i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            ...
            printk("zefdebug snesorid read wrong ,SC202CS i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
            return ERROR_NONE;
            }
        LOG_INF("zef LOG_INF write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
        retry--;
        } while(retry > 0);
        i++;
        retry = 2;
