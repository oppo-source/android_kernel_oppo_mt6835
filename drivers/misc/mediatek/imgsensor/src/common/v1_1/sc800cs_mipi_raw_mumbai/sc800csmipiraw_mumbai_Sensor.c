...
#include "sc800csmipiraw_mumbai_Sensor.h"
#define PFX "sc800cs_truly_main_i_camera_sensor"
#define LOG_INF(format, args...)    \
    pr_err(PFX "[%s] " format, __func__, ##args)
//static unsigned char fusion_id_main[96];
//static unsigned char sn_main[96];
int sc820cs_txd_front_check_flag = 0;

static DEFINE_SPINLOCK(imgsensor_drv_lock);
static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = SC800CS_SENSOR_ID_MUMBAI,
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[6] = {
 ...
 { 3264, 2448,	  0,	0, 3264, 2448, 3264, 2448, 0000,  306, 3264, 1836,    0,	0, 3264, 1836}, //hight speed video
 { 3264, 2448,	  0,	0, 3264, 2448, 3264, 2448, 0000,  306, 3264, 1836,    0,	0, 3264, 1836},// slim video
};

#if 0
#define I2C_BUFFER_LEN 765
static kal_uint16 sc800cs_table_write_cmos_sensor(
...
#define SC800CS_SENSOR_GAIN_BASE             0x400
#define SC800CS_SENSOR_GAIN_MAX              (160 * SC800CS_SENSOR_GAIN_BASE / 10)
#define SC800CS_SENSOR_GAIN_MAP_SIZE  5
static kal_uint16 gain2reg(const kal_uint16 gain)
{
    kal_uint16 reg_gain = gain << 4;
    if (reg_gain < SC800CS_SENSOR_GAIN_BASE)
        reg_gain = SC800CS_SENSOR_GAIN_BASE;
    else if (reg_gain > SC800CS_SENSOR_GAIN_MAX)
        reg_gain = SC800CS_SENSOR_GAIN_MAX;
...
/*  hs04 code for SR-AL6398A-01-12 by liluling at 2022/8/3 end */
    for (gain_index = SC800CS_SENSOR_GAIN_MAP_SIZE - 1; gain_index > 0; gain_index--)
        if (reg_gain >= SC800CS_AGC_Param[gain_index][0])
            break;
    write_cmos_sensor8(0x3e08, SC800CS_AGC_Param[gain_index][1]);
    temp_gain = reg_gain * SC800CS_SENSOR_GAIN_BASE / SC800CS_AGC_Param[gain_index][0];
    write_cmos_sensor8(0x3e07, (temp_gain >> 3) & 0xff);
    pr_debug("Exit! SC800CS_AGC_Param[gain_index][1] = 0x%x, temp_gain = 0x%x, gain = 0x%x, reg_gain = %d\n",
        SC800CS_AGC_Param[gain_index][1], temp_gain, gain, reg_gain);
    return reg_gain;
}               /*      set_gain  */
#if 1
...
static void sensor_init(void)
{
	...
	// write_cmos_sensor8(0x3650,0x21);
	// write_cmos_sensor8(0x3651,0x9c);
	// write_cmos_sensor8(0x3652,0x13);
	// write_cmos_sensor8(0x3653,0x31);
...
static int sc820cs_set_threshold(u8 threshold) //set thereshold
{
	...
	if (threshold < 3 && threshold >= 0) {
		...
		pr_info("sc820cs_otp set_threshold %d\n", threshold);
	} else {
		pr_err("sc820cs_otp set invalid threshold %d\n", threshold);

		return SC820CS_OTP_RET_FAIL;
	}

	return SC820CS_OTP_RET_SUCCESS;
}
...
static int sc820cs_set_page_and_load_data(int page) //set page
{
	...
	while ((read_cmos_sensor(0x4420) & 0x01) == 0x01) {
		delay++;
		pr_info("sc820cs_otp set_page waitting, OTP is still busy for loading %d times\n", delay);
		if (delay == 10) {
			pr_err("sc820cs_otp set_page fail, load timeout!!!\n");

			return SC820CS_OTP_RET_FAIL;
		}
		mdelay(10);
	}
	pr_info("sc820cs_otp set_page success\n");

	return SC820CS_OTP_RET_SUCCESS;
}

static BYTE sc800cs_mumbai_common_data[CAMERA_EEPPROM_COMDATA_LENGTH] = { 0 };
static int read_sc800cs_module_data()
{

    int ret = SC820CS_OTP_RET_FAIL;
    kal_uint16 idx = 0;
    ...
    if (sc820cs_txd_front_otp.ModuleFlag == SC820CS_GROUP1_FLAG){
        ...
        vcmAddr = 0x8285;
        lensAddr = 0x8283;
        sc820cs_set_page_and_load_data(SC820CS_OTP_PAGE2);
    } else if(sc820cs_txd_front_otp.ModuleFlag == SC820CS_GROUP2_FLAG){
        sn_starAddr = 0X8CCE;
        checksumAddr = 0x8CE6;
        vcmAddr = 0x8C97;
        lensAddr = 0x8C95;
        sc820cs_set_page_and_load_data(SC820CS_OTP_PAGE7);
    }

    memset(sc800cs_mumbai_common_data, 0, sizeof(sc800cs_mumbai_common_data));
    // QR
    for(idx = 0; idx < sn_length; idx++)
    {
        sc800cs_mumbai_common_data[8 + idx] = read_cmos_sensor(sn_starAddr + idx);
        sum += sc800cs_mumbai_common_data[8 + idx];
        if(read_cmos_sensor(checksumAddr) == sum%255){
            ret = SC820CS_OTP_RET_SUCCESS;
        } else{
            ret = SC820CS_OTP_RET_FAIL;
        }
    }
    // Vcm ID
    sc800cs_mumbai_common_data[40] = read_cmos_sensor(vcmAddr);
    sc800cs_mumbai_common_data[41] = read_cmos_sensor(vcmAddr + 1);
    // Lens ID
    sc800cs_mumbai_common_data[44] = read_cmos_sensor(lensAddr);
    sc800cs_mumbai_common_data[45] = read_cmos_sensor(lensAddr + 1);

    for (idx = 0; idx < CAMERA_EEPPROM_COMDATA_LENGTH; idx = idx + 4)
    {
        pr_info("imx852 common cam data : %02x %02x %02x %02x\n",
           sc800cs_mumbai_common_data[idx],
           sc800cs_mumbai_common_data[idx + 1],
           sc800cs_mumbai_common_data[idx + 2],
           sc800cs_mumbai_common_data[idx + 3]);
    }
    return ret;
}

static int sc820cs_sensor_otp_read_by_group()
{
	int threshold = 0;
	int ret = SC820CS_OTP_RET_FAIL;
	...
	for (threshold = 0; threshold < 3; threshold++) {
        ...
        sc820cs_set_page_and_load_data(SC820CS_OTP_PAGE2);
        sc820cs_txd_front_otp.ModuleFlag = read_cmos_sensor(SC820CS_OTP_MODULE_FLAGADDR);
        pr_info("sc820cs_otp Read ModuleFlag addr :0x%x, data:0x%x\n", SC820CS_OTP_MODULE_FLAGADDR, sc820cs_txd_front_otp.ModuleFlag);
        ret = read_sc800cs_module_data();
        if(ret == SC820CS_OTP_RET_FAIL)
        {
            pr_err("sc820cs_otp read all otp data in threshold R%d failed\n",threshold);
            continue;
        }
		pr_info("sc820cs_otp read all otp data in threshold R%d success\n", threshold);
		break;
	}
	if (ret == SC820CS_OTP_RET_FAIL) {
		pr_err("sc820cs_otp read otp data  in threshold R1 R2 R3 all failed!\n");
...
static int sc820cs_sensor_otp_read_all_data()
{
	int ret = SC820CS_OTP_RET_FAIL;
	int delay = 0;

	ret = sc820cs_sensor_otp_read_by_group();
	if (ret == SC820CS_OTP_RET_FAIL) {
		pr_err("sc820cs_otp read lsc info in threshold R1 R2 R3 all failed!!!\n");

		return ret;
	}
	pr_info("sc820cs_otp read otp data success\n");
	...
	while ((read_cmos_sensor(0x4420) & 0x01) == 0x01) {
		delay++;
		pr_info("sc820cs_otp read otp is waitting, OTP is still busy for loading %d times\n", delay);
		if (delay == 10) {
			pr_err("sc820cs_otp read otp data fail, load timeout!\n");

			return SC820CS_OTP_RET_FAIL;
		}
		mdelay(10);
	}
	sc820cs_txd_front_check_flag = 1;
	return ret;
}
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
            ...
            do {
                *sensor_id =return_sensor_id();
                if (*sensor_id == imgsensor_info.sensor_id-0x2) {
                    LOG_INF("i2c write id  : 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                    if (sc820cs_txd_front_check_flag == 0) {
					sc820cs_sensor_otp_read_all_data();
				}
                    return ERROR_NONE;
                }
...
static kal_uint32 open(void)
{
    ...
    if (imgsensor_info.sensor_id-0x2 != sensor_id) {
            ...
            UINT8 *feature_para, UINT32 *feature_para_len)
{
    ...
    case SENSOR_FEATURE_GET_MIPI_PIXEL_RATE:
    {
        ...
        LOG_INF("sc800cs SENSOR_FEATURE_GET_MIPI_PIXEL_RATE");
        *(MUINT32 *) (uintptr_t) (*(feature_data + 1)) = rate;
    }
        break;
    case SENSOR_FEATURE_GET_EEPROM_COMDATA:
        memcpy(feature_return_para_32, sc800cs_mumbai_common_data, CAMERA_EEPPROM_COMDATA_LENGTH);
...
UINT32 SC800CS_MIPI_RAW_MUMBAI_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
    ...
    if (pfFunc != NULL)
        *pfFunc =  &sensor_func;
    return ERROR_NONE;
}   /*  SC800CS_TRULY_MAIN_I_MIPI_RAW_SensorInit    */
