...
#include "ov50d40stmipiraw_baikalb5_Sensor.h"
#include "ov50d40_st_Sensor_baikalb5_setting.h"
...
static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = OV50D40_ST_SENSOR_ID_BAIKALB5,
    ...
    .custom2 = {
        .pclk = 100000000, /*1920x1080@60.0fps*/
        .linelength = 1448,
        .framelength = 1150,
        .startx =0,
        .starty = 0,
...
static void read_Eeprom_OVPD(void)
{
    ...
    for (i = 0; i < 450; i ++) {
        ...
        ov50d_OVPD_setting[2*i+1] = read_ov50d40_st_eeprom_module((addr_ovpd+i));
    }
}
...
static kal_uint32 return_sensor_id(void)
{
    return (((read_cmos_sensor(0x300a) << 16) | (read_cmos_sensor(0x300b) << 8) |
              read_cmos_sensor(0x300c)) + SENSOR_ID_OFFSET_BAIKALB5);
}
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            ...
            if (ShineTech_Vendorid == imgsensor_info.module_id){
                *sensor_id = (return_sensor_id() + 0xb5 );
                pr_info("OV50D40 is ShineTech sensorid is  0x%x , imgsensor_info.module_id is 0x%x\n",*sensor_id , imgsensor_info.module_id);
            } else {
                pr_err("OV50D40 is not ShineTech sensorid is  0x%x , imgsensor_info.module_id is 0x%x\n",*sensor_id , imgsensor_info.module_id);
                *sensor_id = 0xFFFFFFFF;
                return ERROR_SENSOR_CONNECT_FAIL;
            }
            if (*sensor_id == imgsensor_info.sensor_id) {
                pr_info("[get_imgsensor_id] OV50D40 ShineTech Read sensor id Success, write id:0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
            ...
            pr_err("[get_imgsensor_id] OV50D40 ShineTech read sensor id fail, write id:0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
            retry--;
        } while (retry > 0);
            i++;
        retry = 2;
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            ...
            sensor_id = (return_sensor_id() + 0xb5 );
            if (sensor_id == imgsensor_info.sensor_id) {
                pr_info("[Open] OV50D40 ShineTech Read sensor id Success, write id:0x%x, id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
                break;
            }
            pr_err("[Open] OV50D40 ShineTech Read sensor id fail, write id:0x%x, id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
            retry--;
        } while (retry > 0);
            i++;
        if (sensor_id == imgsensor_info.sensor_id)
