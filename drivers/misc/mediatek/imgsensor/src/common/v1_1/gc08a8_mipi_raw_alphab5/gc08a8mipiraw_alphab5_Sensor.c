 ...
 * Filename:
 * ---------
 *     gc08a8mipiraw_alphab5_Sensor.c
 *
 * Project:
 * --------
 *	 ALPS
...
#include "gc08a8mipiraw_alphab5_Sensor.h"

static DEFINE_SPINLOCK(imgsensor_drv_lock);
#define DEVICE_VERSION  "gc08a8_alphab5"
#define GC08A8_ALPHAB5_EEPROM_I2C_ADDR 0xA0
#define GC08A8_ECO_VALUE 3
#define MODULE_ID_OFFSET 0x0008
static BYTE gc08a8_alphab5_common_data[CAMERA_EEPPROM_COMDATA_LENGTH] = { 0 };
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = GC08A8_SENSOR_ID_ALPHAB5,
	.module_id = 0x07,
...
static kal_uint16 read_gc08a8_alphab5_eeprom_module(kal_uint32 addr)
{
	kal_uint16 get_byte = 0;
	char pusendcmd[2] = { (char)(addr >> 8), (char)(addr & 0xFF) };

	iReadRegI2C(pusendcmd, 2, (u8 *) &get_byte, 1, GC08A8_ALPHAB5_EEPROM_I2C_ADDR);

	return get_byte;
}
static void read_gc08a8_alphab5_module_data()
{
// gc08a8_alphab5_common_data
// length   : 64
// QR       : [8 ~ 26]
// vcm      : [40 ~ 41]
// lensid   : [44 ~ 45]
// sensorid : [30 ~ 33]
	...
	memset(gc08a8_alphab5_common_data, 0, sizeof(gc08a8_alphab5_common_data));
	// QR
	for(idx = 0; idx < sn_length; idx++)
	{
		gc08a8_alphab5_common_data[8 + idx] = read_gc08a8_alphab5_eeprom_module(sn_starAddr + idx);
	}
	//vcm
	gc08a8_alphab5_common_data[40] = read_gc08a8_alphab5_eeprom_module(vcmAddr);
	gc08a8_alphab5_common_data[41] = read_gc08a8_alphab5_eeprom_module(vcmAddr + 1);
	//lensid
	gc08a8_alphab5_common_data[44] = read_gc08a8_alphab5_eeprom_module(lensAddr);
	gc08a8_alphab5_common_data[45] = read_gc08a8_alphab5_eeprom_module(lensAddr + 1);

	for (idx = 0; idx < CAMERA_EEPPROM_COMDATA_LENGTH; idx = idx + 4)
		pr_info("cam data, idx: %02x %02x %02x %02x %d\n",
		       gc08a8_alphab5_common_data[idx],
		       gc08a8_alphab5_common_data[idx + 1],
		       gc08a8_alphab5_common_data[idx + 2],
		       gc08a8_alphab5_common_data[idx + 3],
			   idx);
}
...
static kal_uint16 gc08a8_init_addr_data[] = {
	0x031c, 0x60,
	0x0337, 0x04,
	0x0335, 0x51,
	0x0336, 0x70,
...
static kal_uint16 gc08a8_3264x1632_addr_data[] = {
	0x031c, 0x60,
	0x0337, 0x04,
	0x0335, 0x51,
	0x0336, 0x70,
...
{
	pr_debug("%s preview_Start\n", __func__);
	table_write_cmos_sensor(gc08a8_3264x1632_addr_data,
		sizeof(gc08a8_3264x1632_addr_data)/sizeof(kal_uint16));
...
{
	kal_uint8 i = 0;
	kal_uint8 tech1_Vendorlensid = 0x82;
	kal_uint8 retry = 2;
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		spin_unlock(&imgsensor_drv_lock);
		do {
                imgsensor_info.module_id = read_gc08a8_alphab5_eeprom_module(MODULE_ID_OFFSET);
                if (tech1_Vendorlensid == imgsensor_info.module_id){
                    *sensor_id = return_sensor_id();
                    pr_info("GC08A8 is tech1 sensorid is  0x%x , imgsensor_info.module_id is 0x%x\n",*sensor_id , imgsensor_info.module_id);
                } else {
                    pr_err("GC08A8 is not tech1 sensorid is  0x%x , imgsensor_info.module_id is 0x%x\n",*sensor_id , imgsensor_info.module_id);
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
				if ((read_gc08a8_version()) < GC08A8_ECO_VALUE) {
					*sensor_id = 0xFFFFFFFF;
					pr_info("[get_imgsensor_id] GC08A8 tech1 alphab5 Read sensor id Success, write id:0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
					return ERROR_SENSOR_CONNECT_FAIL;
				} else {
					...
					if(deviceInfo_register_value == 0x00) {
						...
						read_gc08a8_alphab5_module_data();
						if(otp_data[0] == 0) {
					...
					pr_info("[get_imgsensor_id] GC08A8 tech1 Read sensor id fail, write id:0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
					return ERROR_NONE;
				}
			}
			retry--;
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id();
			if (sensor_id == imgsensor_info.sensor_id) {
				pr_info("[Open] GC08A8 tech1 Read sensor id Success, write id:0x%x, id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
				break;
			}
			pr_info("[Open] GC08A8 tech1 Read sensor id fail, write id:0x%x, id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while (retry > 0);
		i++;
		if (sensor_id == imgsensor_info.sensor_id)
				 ...
				 UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_EEPROM_COMDATA:
		memcpy(feature_return_para_32, gc08a8_alphab5_common_data, CAMERA_EEPPROM_COMDATA_LENGTH);
