...
#include "gc08a8mipiraw_avatarl5_Sensor.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = GC08A8_SENSOR_ID_AVATARL5,
	.module_id = 0x07,
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id();
			printk("avatarl5_front_gc08a8 get_imgsensor_id : 0x%x\n", *sensor_id);
			if (*sensor_id == imgsensor_info.sensor_id) {
				if ((read_gc08a8_version()) < GC08A8_ECO_VALUE) {
					*sensor_id = 0xFFFFFFFF;
					printk("avatarl5_front_gc08a8 get_imgsensor_id failed: 0x%x\n", *sensor_id);
					...
					printk("avatarl5_front_gc08a8 get_imgsensor_id success: 0x%x\n", *sensor_id);
					return ERROR_NONE;
				}
			}
			retry--;
