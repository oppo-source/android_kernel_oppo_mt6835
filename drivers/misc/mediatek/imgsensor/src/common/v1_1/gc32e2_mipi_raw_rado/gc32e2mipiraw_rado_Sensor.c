...
#include "gc32e2mipiraw_rado_Sensor.h"
...
#define DEVICE_VERSION_RADO_GC32E2    "gc32e2"
static DEFINE_SPINLOCK(imgsensor_drv_lock);
static kal_uint8 otp_data[0x4000] = {0};

static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = GC32E2_SENSOR_ID_RADO,
    .module_id = 0x01,
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[9] = {
    ...
    { 6528, 4896, 0, 612, 6528, 3672, 3264, 1836, 0, 0, 3264, 1836, 0, 0, 3264, 1836},  /* custom2 */
...
kal_uint16 gc32e2_rado_init_addr_data[] = {
    0x1320,0x01,
    0x1321,0x00,
    0x0315,0x80,
    0x0aaf,0x00,
...
static void sensor_init(void)
{
    ...
    table_write_cmos_sensor(gc32e2_rado_init_addr_data,
        sizeof(gc32e2_rado_init_addr_data)/sizeof(kal_uint16));
    pr_debug("[%s] init_End\n", __func__);
}   /*    sensor_init  */
...
#define GC32E2_RADO_EEPROM_I2C_ADDR 0xA0
#define GC32E2_ECO_VALUE 3
static BYTE gc32e2_rado_common_data[CAMERA_EEPPROM_COMDATA_LENGTH] = { 0 };

static kal_uint16 read_gc32e2_rado_eeprom_module(kal_uint32 addr)
{
    kal_uint16 get_byte = 0;
    char pusendcmd[2] = { (char)(addr >> 8), (char)(addr & 0xFF) };

    iReadRegI2C(pusendcmd, 2, (u8 *) &get_byte, 1, GC32E2_RADO_EEPROM_I2C_ADDR);

    return get_byte;
}

static void read_gc32e2_rado_module_data()
{
    kal_uint16 idx = 0;
    ...
    memset(gc32e2_rado_common_data, 0,sizeof(gc32e2_rado_common_data));
    // QR
    for(idx = 0; idx < sn_length; idx++)
    {
        gc32e2_rado_common_data[8 + idx] = read_gc32e2_rado_eeprom_module(sn_starAddr + idx);
    }
    // vcm
    gc32e2_rado_common_data[40] = read_gc32e2_rado_eeprom_module(vcmAddr);
    gc32e2_rado_common_data[41] = read_gc32e2_rado_eeprom_module(vcmAddr + 1);
     // lensid
    gc32e2_rado_common_data[44] = read_gc32e2_rado_eeprom_module(lensAddr);
    gc32e2_rado_common_data[45] = read_gc32e2_rado_eeprom_module(lensAddr + 1);
    // macro
    gc32e2_rado_common_data[48] = read_gc32e2_rado_eeprom_module(macAddr_lsb);
    gc32e2_rado_common_data[49] = read_gc32e2_rado_eeprom_module(macAddr_msb);
    // inf
    gc32e2_rado_common_data[52] = read_gc32e2_rado_eeprom_module(infAddr_lsb);
    gc32e2_rado_common_data[53] = read_gc32e2_rado_eeprom_module(infAddr_msb);

    for (idx = 0; idx < CAMERA_EEPPROM_COMDATA_LENGTH; idx = idx + 4)
        printk("cam data: %02x %02x %02x %02x\n",
               gc32e2_rado_common_data[idx],
               gc32e2_rado_common_data[idx + 1],
               gc32e2_rado_common_data[idx + 2],
               gc32e2_rado_common_data[idx + 3]);
}
        ...
        do {
            *sensor_id = (return_sensor_id()+2);
            if (*sensor_id == imgsensor_info.sensor_id) {
                if ((read_gc32e2_rado_eeprom_module(0x00)) == imgsensor_info.module_id) {
                    read_gc32e2_version();
                    if(deviceInfo_register_value == 0x00) {
                        register_imgsensor_deviceinfo("Cam_f", DEVICE_VERSION_RADO_GC32E2, imgsensor_info.module_id);
                        read_gc32e2_rado_module_data();
                        if(otp_data[0] == 0) {
                            read_otp_info(otp_data);
                            gc32e2_rado_otp_identify();
                        } else {
...
/*************************PDAF END*************************/
    ...
    case SENSOR_FEATURE_GET_4CELL_DATA: /*get 4 cell data from eeprom*/
    {
        ...
        if (type == FOUR_CELL_CAL_TYPE_XTALK_CAL)
            gc32e2_rado_copy_crosstalk_data(ct_data);
        break;
    }
    ...
    case SENSOR_FEATURE_GET_EEPROM_COMDATA:
        memcpy(feature_return_para_32, gc32e2_rado_common_data,
