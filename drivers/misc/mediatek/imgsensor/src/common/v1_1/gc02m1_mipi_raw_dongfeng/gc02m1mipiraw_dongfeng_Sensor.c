...
#include "gc02m1mipiraw_dongfeng_Sensor.h"
#define GC02M1_SENSOR_ID_dongfeng 0x2ee
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = GC02M1_SENSOR_ID_dongfeng,
...
static void gc02m1b_table_write_cmos_sensor(kal_uint16 *para, kal_uint32 len)
{
...
{
	return (((read_cmos_sensor(0xf0) << 8) | read_cmos_sensor(0xf1))+ 0x0e);
}
...
{
	LOG_INF("E init setting zzz\n");
	/*system*/
	write_cmos_sensor(0xfc, 0x01);
	write_cmos_sensor(0xf4, 0x41);
	write_cmos_sensor(0xf5, 0xe3); // c0->e3 For Y1&Y6
	...
	write_cmos_sensor(0x17, GC02M1B_MIRROR);
	...
	write_cmos_sensor(0x46, 0x4a);  // 2a->4a For Y1&Y6
	...
	/*ISP*/
	write_cmos_sensor(0xfe, 0x01);
	write_cmos_sensor(0x53, 0x54);  // 44->54 For Y1&Y6
...
{
	write_cmos_sensor(0x41, 0x06);//24fps
	write_cmos_sensor(0x42, 0x3c);
	LOG_INF("E gc02m1 custom1_setting \n");
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == (GC02M1_SENSOR_ID_DONGFENG + 0x01)) {
				printk("GC02M1B i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
				return ERROR_NONE;
			}
			printk("GC02M1B Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
		retry = 2;
