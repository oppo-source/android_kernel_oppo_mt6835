 ...
 * Filename:
 * ---------
 *     gc08a8mipiraw_alphal5_Sensor.c
 *
 * Project:
 * --------
 *	 ALPS
...
#include "gc08a8mipiraw_alphal5_Sensor.h"

static DEFINE_SPINLOCK(imgsensor_drv_lock);
#define DEVICE_VERSION  "gc08a8"
#define GC08A8_ALPHAL5_EEPROM_I2C_ADDR 0xA0
#define GC08A8_ECO_VALUE 3
static BYTE gc08a8_alphal5_common_data[CAMERA_EEPPROM_COMDATA_LENGTH] = { 0 };
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = GC08A8_SENSOR_ID_ALPHAL5,
	.module_id = 0x07,
...
static kal_uint16 read_gc08a8_alphal5_eeprom_module(kal_uint32 addr)
{
	kal_uint16 get_byte = 0;
	char pusendcmd[2] = { (char)(addr >> 8), (char)(addr & 0xFF) };

	iReadRegI2C(pusendcmd, 2, (u8 *) &get_byte, 1, GC08A8_ALPHAL5_EEPROM_I2C_ADDR);

	return get_byte;
}
static void read_gc08a8_alphal5_module_data()
{
// gc08a8_alphal5_common_data
// length   : 64
// QR       : [8 ~ 26]
// vcm      : [40 ~ 41]
// lensid   : [44 ~ 45]
// sensorid : [30 ~ 33]
	...
	memset(gc08a8_alphal5_common_data, 0, sizeof(gc08a8_alphal5_common_data));
	// QR
	for(idx = 0; idx < sn_length; idx++)
	{
		gc08a8_alphal5_common_data[8 + idx] = read_gc08a8_alphal5_eeprom_module(sn_starAddr + idx);
	}
	//vcm
	gc08a8_alphal5_common_data[40] = read_gc08a8_alphal5_eeprom_module(vcmAddr);
	gc08a8_alphal5_common_data[41] = read_gc08a8_alphal5_eeprom_module(vcmAddr + 1);
	//lensid
	gc08a8_alphal5_common_data[44] = read_gc08a8_alphal5_eeprom_module(lensAddr);
	gc08a8_alphal5_common_data[45] = read_gc08a8_alphal5_eeprom_module(lensAddr + 1);

	for (idx = 0; idx < CAMERA_EEPPROM_COMDATA_LENGTH; idx = idx + 4)
		printk("cam data, idx: %02x %02x %02x %02x %d\n",
		       gc08a8_alphal5_common_data[idx],
		       gc08a8_alphal5_common_data[idx + 1],
		       gc08a8_alphal5_common_data[idx + 2],
		       gc08a8_alphal5_common_data[idx + 3],
			   idx);
}
...
static kal_uint32 return_sensor_id(void)
{
	...
	sensor_id = (((read_cmos_sensor(0x03f0) << 8) | read_cmos_sensor(0x03f1)) + SENSOR_ID_OFFSET_ALPHAL5);
	return sensor_id;
}
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id();
			printk("alphal5_front_gc08a8 get_imgsensor_id : 0x%x\n", *sensor_id);
			if (*sensor_id == imgsensor_info.sensor_id) {
				if ((read_gc08a8_version()) < GC08A8_ECO_VALUE) {
					*sensor_id = 0xFFFFFFFF;
					printk("alphal5_front_gc08a8 get_imgsensor_id failed: 0x%x\n", *sensor_id);
					...
					if(deviceInfo_register_value == 0x00) {
						...
						read_gc08a8_alphal5_module_data();
						if(otp_data[0] == 0) {
					...
					printk("alphal5_front_gc08a8 get_imgsensor_id success: 0x%x\n", *sensor_id);
					return ERROR_NONE;
				}
			}
			retry--;
				 ...
				 UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_EEPROM_COMDATA:
		memcpy(feature_return_para_32, gc08a8_alphal5_common_data, CAMERA_EEPPROM_COMDATA_LENGTH);
