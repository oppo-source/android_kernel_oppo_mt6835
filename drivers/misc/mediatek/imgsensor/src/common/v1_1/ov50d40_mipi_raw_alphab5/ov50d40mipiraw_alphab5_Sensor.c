...
#include "ov50d40mipiraw_alphab5_Sensor.h"
#include "ov50d40_Sensor_alphab5_setting.h"
...
#define DEVICE_VERSION_ALPHAB5_OV50D40    "ov50d40_alphab5"
static kal_uint32 streaming_control(kal_bool enable);
static bool _is_binning_size = true;
static DEFINE_SPINLOCK(imgsensor_drv_lock);
static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = OV50D40_SENSOR_ID_ALPHAB5,
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            ...
            if (*sensor_id == imgsensor_info.sensor_id) {
                pr_info("[get_imgsensor_id] OV50D40 sunny alphab5 Read sensor id Success, write id:0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
                if(deviceInfo_register_value == 0x00) {
                    register_imgsensor_deviceinfo("Cam_r", DEVICE_VERSION_ALPHAB5_OV50D40, imgsensor_info.module_id);
            ...
            pr_info("[get_imgsensor_id] OV50D40 sunny Read sensor id fail, write id:0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
            retry--;
        } while (retry > 0);
            i++;
        retry = 2;
