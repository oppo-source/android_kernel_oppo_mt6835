...
#define GC08A8_MUMBAI_OTP_DEBUG  0
#define GC08A8_MUMBAI_I2C_ID     0x62 /*0x20*/
...
static u16 read_cmos_sensor(u16 addr)
{
	...
	iReadRegI2C1(g_pstI2CclientG,pu_send_cmd, 2, (u8 *)&get_byte, 1, GC08A8_MUMBAI_I2C_ID);
...
static void gc08a8_mumbai_otp_init(void)
{
...
static void gc08a8_mumbai_otp_close(void)
{
	write_cmos_sensor(0x0316, 0x01);
	write_cmos_sensor(0x0a67, 0x00);
}

static u16 gc08a8_mumbai_otp_read_group(u16 addr, u8 *data, u16 length)
{
	u16 i = 0;
...
static int gc08a8_mumbai_iReadData(unsigned int ui4_offset,
	unsigned int ui4_length, unsigned char *pinputdata)
{
	...
	gc08a8_mumbai_otp_init();
	mdelay(10);

	i4RetValue = gc08a8_mumbai_otp_read_group(0x15a0, pinputdata,OTP_LENGTH);
