...
#define PFX "CAM_CAL_OMEGAC1FRONT"
...
static unsigned int do_single_lsc_omegac1(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_2a_gain_omegac1(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_lens_id_omegac1(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_pdaf_omegac1(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);

static struct STRUCT_CALIBRATION_LAYOUT_STRUCT cal_layout_table = {
	0x00000006, 0x01A8010A, CAM_CAL_SINGLE_EEPROM_DATA,
	{
		{0x00000001, 0x00000000, 0x00000000, do_module_version},
		{0x00000001, 0x00000000, 0x00000002, do_part_number},
		{0x00000001, 0x00000B00, 0x0000074C, do_single_lsc_omegac1},
		{0x00000001, 0x00000007, 0x0000000E, do_2a_gain_omegac1}, //Start address, block size is useless
		{0x00000001, 0x00001300, 0x00000A3A, do_pdaf_omegac1},
		{0x00000000, 0x00000000, 0x00000000, do_stereo_data},
		{0x00000001, 0x00000000, 0x00008000, do_dump_all},
		{0x00000001, 0x00000008, 0x00000002, do_lens_id_omegac1}
	}
};

struct STRUCT_CAM_CAL_CONFIG_STRUCT omegac1front_op_eeprom = {
	.name = "omegac1front_op_eeprom",
...
#define IMX890_HVBIN_PDAF_PROC1_SIZE  (496)
#define IMX890_HVBIN_PDAF_PROC2_SIZE  (1004)
#define IMX890_HVBIN_PDAF_PROC1_ADDR  (0x1300)
#define IMX890_HVBIN_PDAF_PROC2_ADDR  (0x1500)

unsigned int do_pdaf_omegac1(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	pCamCalData->PDAF.Size_of_PDAF = IMX890_HVBIN_PDAF_PROC1_SIZE + IMX890_HVBIN_PDAF_PROC2_SIZE;
...
//proc1
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			IMX890_HVBIN_PDAF_PROC1_ADDR + IMX890_HVBIN_PDAF_PROC1_SIZE,
...
//proc2
	isValid = 0;
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			IMX890_HVBIN_PDAF_PROC2_ADDR + IMX890_HVBIN_PDAF_PROC2_SIZE,
			1, (unsigned char *)&isValid);
	if(isValid != 1) {
	...
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			IMX890_HVBIN_PDAF_PROC2_ADDR, IMX890_HVBIN_PDAF_PROC2_SIZE,
			(unsigned char *)&pCamCalData->PDAF.Data[IMX890_HVBIN_PDAF_PROC1_SIZE]);
	if (read_data_size > 0)
		err = CAM_CAL_ERR_NO_ERR;
