...
#include "ac388_p_b_a0029_dsi_vdo.h"
#include "oplus_bl.h"
		...
		dcs_write_gce cb, void *handle, bool en)
{
	...
	if (en) {
		...
		reg_count = sizeof(lhbm_pressed_icon_off_cmd_only) / sizeof(struct LCM_setting_table);
		lhbm_pressed_icon_cmd = lhbm_pressed_icon_off_cmd_only;
		for (i = 0; i < reg_count; i++) {
			cb(dsi, handle, lhbm_pressed_icon_cmd[i].para_list, lhbm_pressed_icon_cmd[i].count);
		}
		lcm_setbacklight_cmdq(dsi, cb, handle, oplus_display_brightness);
...
static int panel_doze_enable(struct drm_panel *panel, void *dsi, dcs_write_gce cb, void *handle)
{
	...
	if(oplus_ofp_local_hbm_is_enabled() && oplus_ofp_get_hbm_state()) {
		reg_count = sizeof(lhbm_pressed_icon_off_cmd_only) / sizeof(struct LCM_setting_table);
		lhbm_pressed_icon_off_cmd = lhbm_pressed_icon_off_cmd_only;
		for (i = 0; i < reg_count; i++) {
...
/* #endif  *//* OPLUS_FEATURE_DISPLAY_ONSCREENFINGERPRINT */
	DISP_ERR("%s, ac388_P_B_A0029 lcm_probe+\n", __func__);
	...
	register_device_proc("lcd", "AC388_P_B_A0029", "P_B");
/* #ifdef OPLUS_FEATURE_DISPLAY_ONSCREENFINGERPRINT */
...
static const struct of_device_id lcm_of_match[] = {
	{
	    .compatible = "ac388,p,b,a0029,dsi,vdo",
	},
	{}
};
