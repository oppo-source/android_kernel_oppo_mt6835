...
#include "ac388_p_7_a0025_dsi_vdo.h"
#include "oplus_bl.h"
...
static int lcm_setbacklight_cmdq(void *dsi, dcs_write_gce cb, void *handle, unsigned int level)
{
	...
	if (lhbm_70nit_off_action == 0 && demura_tap != 3) {
		if (mapped_level <= 1147 && demura_tap != 1) {
			demura_tap = 1;
			DISP_ERR("ac388_P_7_a0025 backlight send demura0\n");
			...
			demura_tap = 2;
			DISP_ERR("ac388_P_7_a0025 backlight send demura1\n");
		...
		if (mapped_level <= 1147) {
			demura_tap = 1;
			DISP_ERR("ac388_P_7_a0025 backlight send demura0 , lhbm_70nit_off_action = 1 \n");
			for (i = 0; i < sizeof(dsi_demura0_bl)/sizeof(struct LCM_setting_table); i++){
				cb(dsi, handle, dsi_demura0_bl[i].para_list, dsi_demura0_bl[i].count);
			}
		} else if(mapped_level > 1147) {
			demura_tap = 2;
			DISP_ERR("ac388_P_7_a0025 backlight send demura1 , lhbm_70nit_off_action = 1 \n");
			for (i = 0; i < sizeof(dsi_demura1_bl)/sizeof(struct LCM_setting_table); i++){
				cb(dsi, handle, dsi_demura1_bl[i].para_list, dsi_demura1_bl[i].count);
			}
		}
...
static int oplus_esd_backlight_recovery(void *dsi, dcs_write_gce cb, void *handle)
{
	...
	if (lhbm_70nit_off_action == 0 && demura_tap != 3) {
		if (level <= 1147 && demura_tap != 1) {
			demura_tap = 1;
			DISP_ERR("ac388_P_7_a0025 esd send demura_0\n");
			for (i = 0; i < sizeof(dsi_demura0_bl)/sizeof(struct LCM_setting_table); i++){
				cb(dsi, handle, dsi_demura0_bl[i].para_list, dsi_demura0_bl[i].count);
			}
		} else if(level > 1147 && demura_tap != 2){
			demura_tap = 2;
			DISP_ERR("ac388_P_7_a0025 esd send demura_1\n");
			for (i = 0; i < sizeof(dsi_demura1_bl)/sizeof(struct LCM_setting_table); i++){
				cb(dsi, handle, dsi_demura1_bl[i].para_list, dsi_demura1_bl[i].count);
			}
		}
		...
		dcs_write_gce cb, void *handle, bool en)
{
	...
	if (en) {
		...
		if (lhbm_last_backlight <= 4094 &&  lhbm_last_backlight >= 3516) {
			reg_count = sizeof(lhbm_pressed_icon_on_cmd_first) / sizeof(struct LCM_setting_table);
			lhbm_pressed_icon_cmd = lhbm_pressed_icon_on_cmd_first;
		} else if (lhbm_last_backlight <= 3515 &&  lhbm_last_backlight >= 2839) {
			reg_count = sizeof(lhbm_pressed_icon_on_cmd_second) / sizeof(struct LCM_setting_table);
			lhbm_pressed_icon_cmd = lhbm_pressed_icon_on_cmd_second;
		}else if (lhbm_last_backlight <= 2838 &&  lhbm_last_backlight >= 1871) {
			reg_count = sizeof(lhbm_pressed_icon_on_cmd_third) / sizeof(struct LCM_setting_table);
			lhbm_pressed_icon_cmd = lhbm_pressed_icon_on_cmd_third;
		}else if (lhbm_last_backlight <= 1870 &&  lhbm_last_backlight >= 1155) {
			reg_count = sizeof(lhbm_pressed_icon_on_cmd_fourth) / sizeof(struct LCM_setting_table);
			lhbm_pressed_icon_cmd = lhbm_pressed_icon_on_cmd_fourth;
		}else if (lhbm_last_backlight <= 1154) {
			reg_count = sizeof(lhbm_pressed_icon_on_cmd_fifth) / sizeof(struct LCM_setting_table);
			lhbm_pressed_icon_cmd = lhbm_pressed_icon_on_cmd_fifth;
		}
		...
		} else if (lhbm_last_backlight <= 4094 &&  lhbm_last_backlight >= 1154) {
			reg_count = sizeof(lhbm_pressed_icon_off_cmd_exit_second) / sizeof(struct LCM_setting_table);
			lhbm_pressed_icon_off_cmd = lhbm_pressed_icon_off_cmd_exit_second;
		}
		for (i = 0; i < reg_count; i++) {
...
/* #endif  *//* OPLUS_FEATURE_DISPLAY_ONSCREENFINGERPRINT */
	...
	register_device_proc("lcd", "AC388_P_7_A0025", "P_7");
/* #ifdef OPLUS_FEATURE_DISPLAY_ONSCREENFINGERPRINT */
	oplus_ofp_set_fp_type(&fp_type);
/* #endif  *//* OPLUS_FEATURE_DISPLAY_ONSCREENFINGERPRINT */
	oplus_max_normal_brightness = MAX_NORMAL_BRIGHTNESS;
	check_is_need_fake_resolution(dev);
	DISP_ERR("%s, ac388_P_7 lcm_probe-\n", __func__);
	return ret;
}
...
static const struct of_device_id lcm_of_match[] = {
	{
	    .compatible = "ac388,p,7,a0025,dsi,vdo",
	},
	{}
};
...
MODULE_AUTHOR("like");
MODULE_DESCRIPTION("lcm ac388 Panel Driver");
