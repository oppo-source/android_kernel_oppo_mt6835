	...
	void *handle, unsigned int level)
{
	...
	bl_tb0[2] = level & 0xff;
	if ((level == 0) && (backlight_last_level > 0)) {
		esd_enable = 0;
		if (g_shutdown_flag == 1) {
			...
			pr_info("%s: display off \n", __func__);
			cb(dsi, handle, display_off, ARRAY_SIZE(display_off));
			usleep_range(6000, 6100);
			cb(dsi, handle, dimming_set_off, ARRAY_SIZE(dimming_set_off));
			dimming_on = false;
		}
	} else if ((level > 0) && (backlight_last_level == 0)) {
		pr_info("%s: display on \n", __func__);
		esd_enable = 1;
		usleep_range(6000, 6100);
		cb(dsi, handle, display_on, ARRAY_SIZE(display_on));
	} else if ((level > 0) && (backlight_last_level > 0) && (dimming_on == false)) {
		pr_info("%s: dimming on \n", __func__);
		cb(dsi, handle, dimming_set_on, ARRAY_SIZE(dimming_set_on));
		dimming_on = true;
	}

	pr_info("%s: backlight: %d --> %d \n", __func__, backlight_last_level, level);
