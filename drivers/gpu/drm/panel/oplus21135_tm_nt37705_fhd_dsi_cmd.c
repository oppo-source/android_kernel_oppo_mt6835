...
int oplus_set_dbv_frame(void *dsi, dcs_write_gce_pack cb, void *handle, bool enable)
{
	...
	if (elvss_target >= ELVSS_TARGET_20)
...
int oplus_display_hpwm_onepulse_bl_mapping(int level)
{
	int mapping_level = 0;

	if (level > hpwm_bl && level < hpwm_onepulse_bl_lvl) {
		mapping_level = hpwm_bl +1;
	} else if (level >= hpwm_onepulse_bl_lvl && level <= oplus_max_normal_brightness) {
		mapping_level = backlight_hpwm_onepulse_normal_buf[level - hpwm_onepulse_bl_lvl];
	} else if (level > oplus_max_normal_brightness) {
		mapping_level = backlight_hpwm_onepulse_max_buf[level - oplus_max_normal_brightness - 1];
	} else
		mapping_level = level;

	pr_debug("%s mapping from %d to %d", __func__, level, mapping_level);
...
int oplus_display_panel_set_elvss(void *dsi, dcs_write_gce_pack cb, void *handle, bool en_h_pwm)
{
	...
	if (en_h_pwm) {
		mapping_level = oplus_display_hpwm_onepulse_bl_mapping(oplus_display_brightness);

		if (oplus_display_brightness <= 0x643) {
			pr_info("pwm_turbo dsi_high_12plus backlight level:%d, mapping:%d\n", oplus_display_brightness, mapping_level);
			cmd_size = sizeof(dsi_switch_elvss) / sizeof(struct LCM_setting_table);
			dsi_switch_elvss[cmd_size-1].para_list[1] = mapping_level >> 8;
			dsi_switch_elvss[cmd_size-1].para_list[2] = mapping_level & 0xFF;
			panel_send_pack_hs_cmd(dsi, dsi_switch_elvss, cmd_size, cb, handle);
		} else if (oplus_display_brightness > 0x643) {
			pr_info("pwm_turbo dsi_low_1plus backlight level:%d, mapping:%d\n", oplus_display_brightness, mapping_level);
			cmd_size = sizeof(dsi_switch_elvss_onepulse) / sizeof(struct LCM_setting_table);
			dsi_switch_elvss_onepulse[cmd_size-1].para_list[1] = mapping_level >> 8;
			dsi_switch_elvss_onepulse[cmd_size-1].para_list[2] = mapping_level & 0xFF;
			panel_send_pack_hs_cmd(dsi, dsi_switch_elvss_onepulse, cmd_size, cb, handle);
		}
	} else {
		pr_info("pwm_turbo closed, set dsi_low_3plus backlight level:%d, mapping:%d\n", oplus_display_brightness,mapping_level);
		cmd_size = sizeof(dsi_switch_elvss_low_onepulse) / sizeof(struct LCM_setting_table);
		dsi_switch_elvss_low_onepulse[cmd_size-1].para_list[1] = oplus_display_brightness >> 8;
		dsi_switch_elvss_low_onepulse[cmd_size-1].para_list[2] = oplus_display_brightness & 0xFF;
		panel_send_pack_hs_cmd(dsi, dsi_switch_elvss_low_onepulse, cmd_size, cb, handle);
	}
	...
	if (oplus_ofp_get_aod_state()) {
		if (get_pwm_status()) {
			...
			if (oplus_display_brightness <= 0x643) {
				...
				pr_info("pwm_turbo dsi_low_1plus bl_lvl=%u\n", oplus_display_brightness);
				push_table(ctx, dsi_low_1plus, sizeof(dsi_low_1plus) / sizeof(struct LCM_setting_table));
			}
		} else {
			if (vrefresh_rate == 60) {
...
void oplus_display_panel_set_frequency_pwm(void *dsi, dcs_write_gce cb, void *handle, unsigned int bl_lvl)
{
	...
	if ((bl_lvl <= 0x643 && bl_record > 0x643) || (pwm_power_on == true && bl_lvl <= 0x643)) {
		...
		pr_info("pwm_turbo dsi_low_1plus backlight level=%d, bl_record=%d\n", bl_lvl, bl_record);
		for (i = 0; i < sizeof(dsi_low_1plus)/sizeof(struct LCM_setting_table); i++) {
			cb(dsi, handle, dsi_low_1plus[i].para_list, dsi_low_1plus[i].count);
		}
		bl_record = bl_lvl;
		pwm_power_on = false;
	}
...
static int oplus_display_panel_set_pwm_plus_bl(void *dsi, dcs_write_gce_pack cb, void *handle, unsigned int bl_lvl)
{
	...
	mapping_level = oplus_display_hpwm_onepulse_bl_mapping(bl_lvl);

	if (bl_lvl <= 0x643) {
		dsi_high_12plus_bl[6].para_list[1] = mapping_level >> 8;
		dsi_high_12plus_bl[6].para_list[2] = mapping_level & 0xFF;
		pr_info("pwm_turbo dsi_high_12plus_bl backlight level=%d, mapping=%d, last_backlight=%d\n", bl_lvl, mapping_level, last_backlight);
		...
		cmd_size = sizeof(dsi_low_1plus_bl) / sizeof(struct LCM_setting_table);
		dsi_low_1plus_bl[cmd_size - 1].para_list[1]  = mapping_level >> 8;
		dsi_low_1plus_bl[cmd_size - 1].para_list[2]  = mapping_level & 0xFF;
		pr_info("pwm_turbo dsi_low_1plus_bl backlight level=%d, mapping=%d, last_backlight=%d\n", bl_lvl, mapping_level, last_backlight);
		panel_send_pack_hs_cmd(dsi, dsi_low_1plus_bl, cmd_size, cb, handle);
	}
	return 0;
}
...
static int lcm_setbacklight_cmdq(void *dsi, dcs_write_gce cb, void *handle, unsigned int level)
{
	...
	if (hpwm_mode)
		mapped_level = oplus_display_hpwm_onepulse_bl_mapping(level);
	pr_info("[DISP][INFO][%s:%d]backlight lvl:%u, mapping:%d\n", __func__, __LINE__, level, mapped_level);
...
extern bool aod_hbm_off_handle;
extern int hbm_force_off_when_backlight_0;
extern int hbm_force_off_when_backlight0_flag;
		...
		dcs_write_gce cb, void *handle, bool en)
{
	...
	if (vrefresh_rate == 60) {
		if (en) {
			if (hpwm_mode == 1) {
				hbm_cmd = hbm_on_onepulse_cmd_60hz;
				reg_count = sizeof(hbm_on_onepulse_cmd_60hz) / sizeof(struct LCM_setting_table);
			} else if (hpwm_mode == 0) {
		...
		if (en) {
			if (hpwm_mode == 1) {
				hbm_cmd = hbm_on_onepulse_cmd_90hz;
				reg_count = sizeof(hbm_on_onepulse_cmd_90hz) / sizeof(struct LCM_setting_table);
			} else if (hpwm_mode == 0) {
		...
		if (en) {
			if (hpwm_mode == 1) {
				hbm_cmd = hbm_on_onepulse_cmd_120hz;
				reg_count = sizeof(hbm_on_onepulse_cmd_120hz) / sizeof(struct LCM_setting_table);
			} else if (hpwm_mode == 0) {
	...
	if (!en) {
		...
		if (aod_hbm_off_handle == true) {
			pr_info("enter doze after hbm off, filter backlight setting\n");
			return 0;
		}
		lcm_setbacklight_cmdq(dsi, cb, handle, oplus_display_brightness);
...
static int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	u32 pwm_bl_config = 0;
	u32 pwm_support = 0;
	u32 hbm_force_off_when_backlight_set0_flag = 0;
#ifdef OPLUS_FEATURE_DISPLAY_ONSCREENFINGERPRINT
	unsigned int fp_type = 0x108;
#endif /* OPLUS_FEATURE_DISPLAY_ONSCREENFINGERPRINT */
	...
	if (oplus_temp_compensation_init_onepulse(dev)){
		TEMP_COMPENSATION_ERR("failed to init temp compensation onepulse\n");
		return -ENODEV;
	}
#endif /* OPLUS_FEATURE_DISPLAY_TEMP_COMPENSATION */
	...
	rc = of_property_read_u32(dev->of_node, "oplus-hbm-force-off-when-backlight-set0-flag", &hbm_force_off_when_backlight_set0_flag);
	if (rc == 0) {
		hbm_force_off_when_backlight0_flag = hbm_force_off_when_backlight_set0_flag;
		pr_info("lcm_probe hbm_force_off_when_backlight0_flag=0x%x\n", hbm_force_off_when_backlight0_flag);
	} else {
		hbm_force_off_when_backlight0_flag = 0;
		pr_info("lcm_probe hbm_force_off_when_backlight0_flag=%d\n", hbm_force_off_when_backlight0_flag);
	}
	...
	oplus_ofp_get_panel_cap_config(dev);
#endif /* OPLUS_FEATURE_DISPLAY_ONSCREENFINGERPRINT */
