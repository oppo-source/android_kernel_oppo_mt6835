...
#endif
/* 2024/6/19, add sia91xx_v3_1 driver flag */
static bool is_sia91xx_v3_1_flag = 0;
#if IS_ENABLED(CONFIG_SND_SOC_SIA91XX_V3_1_0)
...
#if IS_ENABLED(CONFIG_OPLUS_MTK_AUDIO_EXT)
/*2025/05/17, Add for oplus extend audio*/
					...
					int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		#ifdef OPLUS_FEATURE_SPEAKER_MUTE
		if (speaker_mute_control) {
			dev_info(card->dev, "%s(), speaker force mute\n", __func__);
			return 0;
		}
...
static int mt6835_mt6377_dev_probe(struct platform_device *pdev)
{
	...
	ret = of_property_read_u32(pdev->dev.of_node,
			"si,is_sia91xx_v3_1", &sia91xx_v3_driver_flag);
	if (ret) {
		dev_info(&pdev->dev, "%s() read prop si,is_sia91xx_v3_1 fail\n", __func__);
		is_sia91xx_v3_1_flag = false;
	} else {
		dev_info(&pdev->dev, "%s() read prop si,is_sia91xx_v3_1 %d\n", __func__, sia91xx_v3_driver_flag);
		is_sia91xx_v3_1_flag = sia91xx_v3_driver_flag;
	}
...
#if IS_ENABLED(CONFIG_OPLUS_MTK_AUDIO_EXT)
       /*Add for oplus extend audio*/
	extend_codec_i2s_be_dailinks(mt6835_mt6377_dai_links, ARRAY_SIZE(mt6835_mt6377_dai_links));
#endif /* CONFIG_OPLUS_MTK_AUDIO_EXT */

	card->dev = &pdev->dev;
#if IS_ENABLED(CONFIG_SND_SOC_SIA91XX_V3_1_0)
    if (is_sia91xx_v3_1_flag) {
...
#else
    soc_aux_init_only_sia81xx(pdev, card);
#endif /*CONFIG_SND_SOC_SIA91XX_V3_1_0*/
